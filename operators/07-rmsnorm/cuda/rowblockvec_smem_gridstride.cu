...
#ifndef GRID_STRIDE_SM_MULT
#define GRID_STRIDE_SM_MULT 4
#endif

#if VEC_SIZE != 4
#error "rowblockvec_smem_gridstride currently supports VEC_SIZE=4 only"
#endif

#if GRID_STRIDE_SM_MULT <= 0
#error "GRID_STRIDE_SM_MULT must be positive"
#endif
...
__global__ void rmsnorm_rowblock_vec_smem_gridstride_kernel(
    ...
    // rowblockvec_smem + grid-stride rows：固定 block 数为 SM 数的若干倍，每个 block 跨步处理多行。
...
void solve(
    ...
    // 固定 block 数为 SM 数 * GRID_STRIDE_SM_MULT，过大的 M 通过 kernel 内 grid-stride row loop 覆盖。
    ...
    int target_blocks = prop.multiProcessorCount * GRID_STRIDE_SM_MULT;
    if(target_blocks < 1){
        target_blocks = 1;
    }

    dim3 blocksPerGrid(M < target_blocks ? M : target_blocks);
    size_t shared_bytes = static_cast<size_t>(N) * sizeof(__nv_bfloat16);

    rmsnorm_rowblock_vec_smem_gridstride_kernel<<<blocksPerGrid, threadsPerBlock, shared_bytes>>>(
...
PYBIND11_MODULE(TORCH_EXTENSION_NAME, m) {
    ...
    m.def("solve", &solve, "RMSNorm Row Block Vectorized Shared Memory Grid-Stride Kernel",
          py::arg("X"),
          py::arg("gamma"),
          py::arg("Y"),
          py::arg("M"),
