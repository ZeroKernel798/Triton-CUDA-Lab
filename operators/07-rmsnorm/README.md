...
3. rowblockvec_smem_gridstride.cu：在 rowblockvec_smem 基础上使用 grid-stride row loop。launcher 通过 `cudaGetDeviceProperties` 获取 SM 数，并将实际 block 数限制为 `SM * GRID_STRIDE_SM_MULT`（不超过 `M`）；`GRID_STRIDE_SM_MULT` 是编译宏，当前在 `test_cfg.py` 中 sweep `2/4/6/8` 倍，适合 `M` 很大时减少过量 block 调度开销。该版本还会在进入 row loop 前把 `gamma` 缓存到 shared memory，避免同一个 block 处理多行时重复从 global memory 读取权重；当前 grid-stride 配置使用 8 个 bf16 元素向量化读写。
