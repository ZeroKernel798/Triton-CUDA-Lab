...
def application(
    ...
    # 1. Swizzling 坐标计算 (保持原样，这部分逻辑没问题)
    pid = tl.program_id(0)
...
def solve(A: torch.Tensor, B: torch.Tensor, C: torch.Tensor, M: int, N: int, K: int, **kwargs):
    """
    用于被你的 OperatorSpec 自动调用进行调优的接口
    """
...
def solve(A: torch.Tensor, B: torch.Tensor, C: torch.Tensor, M: int, N: int, K: int, **kwargs):
    ...
    try:
        ...
        if "out of resource" in error_msg or "shared memory" in error_msg:
            # 也可以在这里 print 一下，知道哪些配置被跳过了
            # print(f"  [Auto-Skip] Config {kwargs} exceeded hardware limits.")
            return None 
        else:
            # 如果是代码写错了（语法错误、空指针等），依然要抛出，不能吞掉
            raise e