...
#     target_op = "04-softmax-attention"
...
def run_lab(op_folder, epoch=1000, warm_up=True, warm_epoch=100):
    ...
    cu_files = glob.glob(f"operators/{op_folder}/cuda/*.cu")
    for cu_file in cu_files:
        print(f"🛠️  Compiling CUDA: {os.path.basename(cu_file)}")
        try:
            lib = KernelEngine.setup_cuda(cu_file)
            all_pass = True
            ...
            # A. 功能测试
            for case in func_cases:
                args, types = spec.get_cuda_args(case)
                # 统一清理所有输出张量
                spec.clear_outputs(case)
                
                # 转换参数为 Ctypes
                c_args = [ctypes.cast(a.data_ptr(), ctypes.c_void_p) if isinstance(a, torch.Tensor) 
                          else t(a) for a, t in zip(args, types)]
                
                lib.solve(*c_args)
                
                # 让 spec 自己校验精度
                if not spec.validate(case):
                    print(f"   - ❌ Func Fail")
                    all_pass = False; break
            ...
            if all_pass:
                ...
                logger.record(os.path.basename(cu_file), avg_ms, True)
                print(f"   - Status: ✅ PASS | Time: {avg_ms:.4f} ms")
        except Exception as e: print(f"   - 💥 CUDA Error: {str(e)}")
    ...
    for py_file in triton_files:
        ...
        try:
            ...
            for case in func_cases:
                t_args = spec.get_triton_args(case)
                grid = lambda meta: spec.get_triton_grid(case, meta)
                spec.clear_outputs(case)
            ...
            if all_pass:
                t_args = spec.get_triton_args(perf_case)
                grid = lambda meta: spec.get_triton_grid(perf_case, meta)
                ...
                logger.record(os.path.basename(py_file), avg_ms, True)
                print(f"   - Status: ✅ PASS | Time: {avg_ms:.4f} ms")
        except Exception as e: print(f"   - 💥 Triton Error: {str(e)}")

    logger.plot(op_folder)

if __name__ == "__main__":
    target_op = sys.argv[1] if len(sys.argv) > 1 else "04-softmax-attention"
    run_lab(target_op)
